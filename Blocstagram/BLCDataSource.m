...
+ (NSString *) instagramClientID {
    return @"7e4aa3d284394933b9d586536179c4a1";
}
...
- (void) toggleLikeOnMediaItem:(BLCMedia *)mediaItem {
    ...
    if (mediaItem.likeState == BLCLikeStateNotLiked) {
        ...
        [self.instagramOperationManager DELETE:urlString parameters:parameters
                                       success:^(AFHTTPRequestOperation *operation, id responseObject) {
                                           mediaItem.likeState = BLCLikeStateNotLiked;
                                           [self reloadMediaItem:mediaItem];
                                       }
...
- (void) commentOnMediaItem:(BLCMedia *)mediaItem withCommentText:(NSString *)commentText {
    ...
    [self.instagramOperationManager POST:urlString parameters:parameters
        ...
        NSDictionary *parameters = @{@"access_token": self.accessToken};
        [self.instagramOperationManager GET:refreshMediaUrlString parameters:parameters
                                    success:^(AFHTTPRequestOperation *operation, id responseObject) {
